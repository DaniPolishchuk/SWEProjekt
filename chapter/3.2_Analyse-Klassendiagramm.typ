...
Die Klasse `Mitarbeiter` bildet die zentrale Personalentität des Bauunternehmens ab. Sie enthält alle relevanten Stammdaten: Mitarbeiternummer, Vorname, Nachname, Geburtsdatum, Kontaktdaten (E-Mail, Telefonnummer) sowie die Beschäftigungsart (Vollzeit oder Teilzeit), Vertragsbeginn und Vertragsende (LF 20, LF 90). Das Attribut `position` unterscheidet zwischen den im Lastenheft genannten Mitarbeitertypen: Projektleiter, Bauleiter, Baugruppenleiter, Vorarbeiter, gelernter/ungelernter Bauarbeiter und Verwaltungsmitarbeiter.
...
Die Klasse `Adresse` kapselt die Adressinformationen (Straße, Hausnummer, PLZ, Ort, Land) und wird von mehreren Klassen referenziert: Mitarbeiter (Wohnadresse), Projekte und Arbeitsaufträge (Einsatzort), Lager (Lagerstandort) sowie Unterauftragnehmer (Firmenadresse). Durch diese zentrale Modellierung wird Redundanz vermieden.

=== Rolle (Berechtigungsklasse)

Die Klasse `Rolle` definiert die Benutzerrolle eines Mitarbeiters im System. Die vordefinierten Rollen (Administrator, Verwaltungsmitarbeiter, Bauleiter, Bauarbeiter, Vorarbeiter, Mitarbeiter) bestimmen die Zugriffsrechte gemäß der Berechtigungstabelle aus der Lastenheftanalyse. Jeder Mitarbeiter hat genau eine Rolle. Es handelt sich hierbei um eine Berechtigungsklasse zur Zugriffssteuerung, nicht um das Analysemuster "Rolle" im Sinne der Vorlesung -- dieses wird stattdessen durch die mehreren benannten Assoziationen zwischen `Mitarbeiter` und anderen Klassen realisiert (siehe Abschnitt Analysemuster).
...
Die Klasse `Projekt` bildet die übergeordnete organisatorische Einheit für Bauvorhaben ab. Ein Projekt hat einen verantwortlichen Projektleiter (Assoziation zu `Mitarbeiter`), einen Einsatzort (Assoziation zu `Adresse`) sowie Start- und Endtermine. Die Beziehung zu `Arbeitsauftrag` ist als n:m-Beziehung modelliert: Ein Projekt kann null oder mehrere Arbeitsaufträge enthalten (ein neu angelegtes Projekt kann zunächst ohne Aufträge existieren), und ein Arbeitsauftrag kann mehreren Projekten zugeordnet sein.

=== Arbeitsauftrag

Die Klasse `Arbeitsauftrag` enthält alle wesentlichen Auftragsdaten (LF 30). Zentrale Attribute sind die Auftragsbezeichnung, der Bauplan (als Dateipfad gemäß Vereinfachung), Start- und Endtermine, der Kostenvoranschlag (aus dem Finanzsystem lesend) und der Status. Dem Arbeitsauftrag werden beteiligte Personen (Assoziation zu `Mitarbeiter`, 1..\*) und ein Einsatzort (Assoziation zu `Adresse`) zugeordnet. Zwischentermine verweisen über eine unidirektionale Assoziation auf den Arbeitsauftrag (siehe Klasse `Termin`).

=== Unterauftrag

Die Klasse `Unterauftrag` modelliert die hierarchische Gliederung von Arbeitsaufträgen (LF 40). Unteraufträge werden per Komposition dem übergeordneten Arbeitsauftrag zugeordnet und können einem externen Unterauftragnehmer zugewiesen sein. Sie erben den Einsatzort und das Projekt vom Arbeitsauftrag, können aber eigene Termine und Kosten haben.

=== Unterauftragnehmer

Die Klasse `Unterauftragnehmer` verwaltet die Kontaktdaten externer Firmen, die Unteraufträge ausführen (LF 40). Sie enthält Firmenname, Ansprechperson, Kontaktdaten und Fachbereich und verweist über eine Assoziation auf eine `Adresse`.

=== Termin

Die Klasse `Termin` modelliert einzelne Termine und Zwischentermine innerhalb der Projektplanung (LF 40, gemäß Vereinfachung als einfache Klasse modelliert). Jeder Termin hat ein Datum, eine optionale Uhrzeit, eine Bezeichnung und eine Beschreibung. Über unidirektionale Assoziationen kann ein Termin einem Arbeitsauftrag, einem Projekt oder einer Buchung zugeordnet werden. Dadurch wird die im Lastenheft geforderte Übersicht über alle terminrelevanten Daten ermöglicht, ohne Referenzen als Attribute in der Klasse zu speichern.

=== Rechnung

Die Klasse `Rechnung` enthält die aus dem Finanzbuchhaltungssystem lesend übernommenen Rechnungsdaten (Rechnungsnummer, Betrag, Fälligkeitsdatum, Status). Rechnungen sind per Assoziation einem Arbeitsauftrag zugeordnet. Der Zugriff erfolgt unidirektional und ausschließlich lesend über das externe Finanzbuchhaltungssystem.
...
Die Klasse `Lager` repräsentiert die Lagerorte der Geräte (LF 50). Lager können als Platz (Außengelände) oder Gebäude (Lagerhalle) typisiert sein und verfügen über eine Adresse (Assoziation zu `Adresse`), eine optionale Grundstücksbezeichnung und eine optionale Kapazitätsangabe. Jedes Geräteexemplar ist genau einem Lager zugeordnet.

=== Buchung (Koordinator-Muster)

Die Klasse `Buchung` realisiert das Koordinator-Muster (LF 50). Sie verknüpft ein oder mehrere Geräteexemplare mit einem Arbeitsauftrag und trägt eigene Attribute: Buchungszeitraum (Start- und Enddatum), Buchungsdatum und Status (Aktiv, Abgeschlossen, Storniert). Zusätzlich wird der buchende Mitarbeiter referenziert. Durch die Buchung wird die Verfügbarkeit der Geräte gesteuert und die im Lastenheft geforderte Verfügbarkeitssuche ermöglicht.

=== Bild

Die Klasse `Bild` ermöglicht die Zuordnung beliebig vieler Bilder mit Titel zu Mitarbeitern, Arbeitsaufträgen, Projekten und Geräteexemplaren (LF 80). Jedes Bild besitzt die Attribute Bild-ID, Titel, Dateipfad, Dateiname, Elementtyp (zur Filterung nach zugeordnetem Objekttyp), Hochladedatum, Dateigröße und Format. Bilder werden per Komposition den jeweiligen Elementen zugeordnet -- dies drückt die Zugehörigkeit aus: Ein Bild existiert nur im Kontext seines zugeordneten Elements und wird bei dessen Löschung mitentfernt. Unabhängig davon verweist die separate Hochlader-Assoziation auf den Mitarbeiter, der das Bild hochgeladen hat. Diese zweite Assoziation ist semantisch verschieden von der Komposition: Während die Komposition die Besitzbeziehung abbildet, dokumentiert die Hochlader-Assoziation lediglich den Urheber des Uploads.
