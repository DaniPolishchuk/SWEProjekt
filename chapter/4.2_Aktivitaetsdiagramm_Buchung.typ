...
Im vorliegenden Kapitel wird das Verhalten der Verwaltungssoftware anhand des Szenarios "Gerät anlegen und buchen" (im Datenmodell konsequent als Gerät geführt; das ursprünglich verwendete Wort "Baumaschine" bezeichnet lediglich einen der beiden Werte des Typattributs am `Geräte-Typ`, konkret `BAUMASCHINE` vs. `BAUWERKZEUG`, und ist keine eigene Klasse) als UML-Aktivitätsdiagramm modelliert. Die GUI-Beschriftung "Geräte & Maschinen" folgt derselben Vereinfachung und dient ausschließlich der benutzerfreundlichen Anzeige; im Modell existieren nur die Klassen `Gerät` und `Geräte-Typ` sowie die Enumeration `GerätKategorie` (Bagger, LKW, Kran, Rüttler, Bohrmaschine, Schalungsteil, Zaun, Bausicherung). Während das Sequenzdiagramm in @chapter-Sequenzdiagramm-Szenariobetrachtung den Nachrichtenaustausch zwischen Lebenslinien in zeitlicher Reihenfolge darstellt, visualisiert das Aktivitätsdiagramm den Kontrollfluss mit Entscheidungen und Schleifen. Es eignet sich damit besonders gut zur Darstellung von Abläufen, die mehrere Akteure und Verantwortlichkeitsbereiche umfassen -- hier konkret den Verwaltungsmitarbeiter beim Anlegen von Geräten sowie den Bau-/Projektleiter beim Anlegen einer Buchung.
