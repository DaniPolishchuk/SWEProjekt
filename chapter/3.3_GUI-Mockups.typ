...
In der rechten Spalte werden ergänzende Daten erfasst, die teils mehrere Felder gleichzeitig betreffen. Der Einsatzort wird strukturiert in Straße, Hausnummer, Postleitzahl und Ort eingegeben, was eine spätere automatische Auswertung -- etwa zur standortbasierten Suche nach verfügbaren Geräten -- erheblich vereinfacht. Die beteiligten Personen werden als Mehrfachauswahl mit Chip-Darstellung erfasst; bereits ausgewählte Mitarbeiter erscheinen als kompakte, mit einem Schließsymbol versehene Pillen. Das im Mockup separat sichtbare Feld "Bauleiter" ist streng genommen kein zusätzliches Attribut der Klasse `Arbeitsauftrag`, sondern eine hervorgehobene Auswahl unter den beteiligten Personen: Es kennzeichnet, welcher der beteiligten Mitarbeiter (mit Position `BAULEITER`) für die operative Verantwortung dieses Auftrags zuständig ist. Auf Datenmodellebene wird diese Verantwortung über die bereits vorhandene Assoziation "beteiligte Personen" zusammen mit der Enumeration `Position` abgebildet; das GUI-Feld unterstützt lediglich die häufige Ergonomie-Anforderung, den Bauleiter separat hervorzuheben, und ist standardmäßig mit dem aktuell angemeldeten Mitarbeiter vorbelegt, sofern dieser die entsprechende Position innehat.
