...
Am oberen Bildschirmrand wurde eine über alle Ansichten hinweg identisch aufgebaute Topbar implementiert, die beim vertikalen Scrollen am Bildschirm verankert bleibt. Sie enthält von links nach rechts den aktuellen Brotkrumenpfad zur Verortung der gerade geöffneten Ansicht innerhalb der Anwendung, ein Glockensymbol für Systemmeldungen und Hinweise, einen Sprachumschalter zur Erfüllung der im Lastenheft geforderten Mehrsprachigkeit sowie ein Profilelement mit Avatarbild und Klarnamen des angemeldeten Mitarbeiters. Über das Profilelement lässt sich ein Aufklappmenü für persönliche Einstellungen und das Abmelden vom System öffnen.
...
Für die Kennzeichnung von Zuständen wurde eine durchgängige Statusfarbpalette eingeführt: Grün signalisiert positive oder verfügbare Zustände (etwa "Verfügbar" bei Geräten oder "Abgeschlossen" bei Aufträgen), Bernstein bzw. Gelb verweist auf Übergangs- oder Wartezustände (etwa "In Arbeit" oder "In Wartung"), Rot kennzeichnet kritische oder blockierende Zustände (etwa "Defekt" oder "In Verzug") und ein gedämpftes Grau bzw. Blau wird für neutrale Zustände wie "Pausiert", "Geplant" oder "Storniert" verwendet. Die Statuspills sind über sämtliche Ansichten hinweg identisch gestaltet und dadurch sofort wiedererkennbar.

=== Rollenbasierte Sichtbarkeit
Die in diesem Kapitel gezeigten Mockups bilden überwiegend die Sicht eines Bauleiters ab, da diese Rolle den größten Funktionsumfang im Arbeitsalltag aufweist und damit zu den am häufigsten genutzten Sichten innerhalb der Anwendung zählt. Funktionen, die laut Berechtigungstabelle ausschließlich dem Administrator vorbehalten sind -- etwa der Import und Export von Daten oder die rollenweite Backup-Erstellung -- erscheinen in der dargestellten Sicht zwar visuell, sind im produktiven Einsatz jedoch nur für den Administrator tatsächlich sichtbar. In den Sichten anderer Rollen wären diese Schaltflächen entweder ausgeblendet oder deaktiviert. Die Mockups zeigen aus Übersichtsgründen die maximale Funktionsausprägung; die rollenspezifische Reduktion erfolgt zur Laufzeit über die Berechtigungsklasse.
...
Der Hauptarbeitsbereich des Dashboards ist horizontal in zwei Spalten unterteilt. Auf der linken, breiteren Seite findet sich eine umschaltbare Tagesübersicht mit den Karteireitern "Termine", "Aufträge" und "Buchungen". Im standardmäßig geöffneten Reiter "Termine" sind die Termine des aktuellen Tages chronologisch in einer vertikal verlaufenden Stundenleiste angeordnet. Jeder Termin ist farblich nach seiner Kategorie codiert (Haupttermin, Zwischentermin, interner Termin sowie Telefontermine mit externen Beteiligten) und zeigt neben dem Titel die zugehörige Auftragsnummer als verlinkte Referenz. Durch einen Klick auf einen Termin gelangt der Anwender in die Detailansicht im Terminplaner.
...
Es folgen die Felder zur Zuordnung des übergeordneten Projekts (mit der Möglichkeit, an gleicher Stelle ein neues Projekt anzulegen), der Pfad zum zugehörigen Bauplan sowie die drei zentralen Termindaten Starttermin, Endtermin und optionaler Zwischentermin. Die Termindaten werden über klickbare Datumsfelder erfasst, die einen kompakten Kalenderpicker einblenden.

In der rechten Spalte werden ergänzende Daten erfasst, die teils mehrere Felder gleichzeitig betreffen. Der Einsatzort wird strukturiert in Straße, Hausnummer, Postleitzahl und Ort eingegeben, was eine spätere automatische Auswertung -- etwa zur standortbasierten Suche nach verfügbaren Geräten -- erheblich vereinfacht. Die beteiligten Personen werden als Mehrfachauswahl mit Chip-Darstellung erfasst; bereits ausgewählte Mitarbeiter erscheinen als kompakte, mit einem Schließsymbol versehene Pillen. Das Feld "Bauleiter" weist die operative Verantwortung dem die Eingabe tätigenden Bauleiter zu und ist standardmäßig mit dem aktuell angemeldeten Mitarbeiter vorbelegt, was den häufigsten Anwendungsfall optimal unterstützt.
...
Die linke Spalte beherbergt die Master-Liste sämtlicher Geräte. An der Spitze der Liste steht eine Karteireiterleiste, die zwischen "Alle", "Baumaschinen" und "Bauwerkzeuge" unterscheidet -- entsprechend der im Lastenheft genannten Trennung zwischen schweren Maschinen (Bagger, LKWs, Kräne, Rüttler, Bohrmaschinen) und leichten Werkzeugen (Schalungsteile, Bausicherungen, Zäune). Die jeweilige Anzahl der Treffer wird zur Orientierung in Klammern hinter dem Reiternamen angezeigt. Direkt darunter befindet sich eine durchgehende Filter- und Suchleiste, über die nach Gerätenummer, Bezeichnung oder Seriennummer gesucht werden kann; ergänzend stehen Filter für Status, Kategorie und Lager zur Verfügung. Die eigentliche Geräteliste wird als vertikal gestapelte Reihe kompakter Einträge dargestellt; jeder Eintrag zeigt ein kleines Vorschaubild des Geräts, die Gerätebezeichnung, die mono-typografisch gesetzte Gerätenummer mit Kategorieangabe sowie eine Statuspille am rechten Rand. Der gerade ausgewählte Eintrag ("Bagger CAT 320") ist über einen orangefarbenen Akzentbalken am linken Rand und eine geringfügig hellere Hintergrundfarbe hervorgehoben.
...
Im darunterliegenden Karteireiterbereich ("Übersicht", "Buchungen", "Wartung", "Ausrüstung", "Standort \& Lager", "Historie") können sämtliche weiterführenden Informationen zum Gerät erschlossen werden. Im standardmäßig geöffneten Reiter "Übersicht" sind die Stammdaten des Exemplars mit interner Nummer und Seriennummer aufgelistet, ergänzt um den Bereich "Standort \& Lager" mit kartografischer Darstellung und einer kompakten Lageradresse sowie den Bereich "Zubehör (Baugruppe-Muster)", in dem das dem Gerät zugeordnete Zubehör (Tieflöffel, Hydraulikhammer und ähnliche Anbauteile) verwaltet wird. Diese Zubehörverwaltung realisiert das im Lastenheft (LF 50) geforderte Konzept zur Verwaltung kompatibler Anbaugeräte.

#pagebreak()

== Buchung mit Verfügbarkeitssuche
Die Buchungsansicht (siehe @mockup_buchung) realisiert den dreistufigen Buchungsprozess für Geräte und Maschinen und stellt damit eine der zentralen Funktionalitäten zur Erfüllung von LF 50 dar. Sie wird modal über sämtliche Hauptansichten geöffnet, sodass der Buchungsvorgang aus jedem Kontext heraus angestoßen werden kann -- sei es aus der Detailansicht eines Auftrags, aus der Geräteverwaltung oder direkt aus dem Terminplaner.
...
Auf Grundlage dieser Eingaben präsentiert die Anwendung im darunter liegenden Bereich eine sortierte Liste verfügbarer Geräte. Die Sortierung erfolgt standardmäßig kombiniert nach Entfernung zum Einsatzort und Verfügbarkeit, was eine optimale Ressourcennutzung sicherstellt. Jede Ergebnis-Karte zeigt die Bezeichnung und Gerätenummer des Geräts, das zugehörige Lager mit Entfernung in Kilometern sowie den konkreten Verfügbarkeitsstatus im angefragten Zeitraum. Vollständig verfügbare Geräte sind mit einer grünen Statuspille und einem Eintrag "Im Zeitraum frei: 100\%" gekennzeichnet, teilweise verfügbare Geräte zeigen eine bernsteinfarbene Pille mit detaillierter Aufschlüsselung der freien und belegten Tage, defekte oder in Wartung befindliche Geräte werden mit einer roten Pille kenntlich gemacht und sind über ein deaktiviertes Auswahlfeld nicht buchbar. Das aktuell ausgewählte Gerät ist durch einen orangefarbenen Rahmen hervorgehoben.
...
Die Kopfzeile zeigt den Titel der Ansicht zusammen mit der aktuell dargestellten Kalenderwoche ("KW 21 $dot.c$ 18.05. -- 24.05.2026"). Rechts daneben befindet sich eine dreistufige Umschalter zur Wahl der Granularität (Tages-, Wochen- oder Monatsansicht), zwei Pfeilschaltflächen zur Navigation in die Vergangenheit beziehungsweise Zukunft sowie eine Schaltfläche "Heute" zum schnellen Sprung auf den aktuellen Tag. Eine darunter liegende Filterzeile mit den Filtern "Mitarbeiter", "Kategorie" und "Auftrag" sowie zwei Umschaltern für die Anzeige von Geräte-Buchungen und Urlauben erlaubt die individuelle Anpassung der Kalenderansicht an den jeweiligen Informationsbedarf.

Den Hauptteil der Ansicht bildet das Wochenraster mit einer vertikalen Stundenachse von 06:00 Uhr bis 21:00 Uhr und sieben horizontal angeordneten Tagesspalten. Der aktuelle Tag (im dargestellten Beispiel der Donnerstag, 21. Mai 2026) ist durch eine orangefarbene Kennzeichnung der Spaltenbeschriftung sowie eine geringfügig hellere Hintergrundfarbe der gesamten Spalte visuell hervorgehoben. Termine werden als rechteckige, farbcodierte Blöcke innerhalb der Spalten platziert, wobei sich die vertikale Position und Höhe aus dem Beginn und der Dauer des Termins ableiten. Die Farbcodierung folgt dem im Lastenheft Q\&A definierten Schema und unterscheidet vier Termintypen: Haupttermine (grün), Zwischentermine (orange), interne Termine (gelb) sowie Urlaubseinträge (lila mit Streifenmuster), wie sie in der ergänzenden Legende am unteren Rand der Ansicht erläutert werden. Geräte-Buchungen werden ebenfalls als Kalendereinträge dargestellt, jedoch mit einem zusätzlichen Buchungssymbol versehen, sodass sie sich visuell von gewöhnlichen Terminen unterscheiden lassen.

Auf der rechten Seite der Ansicht wurde ein Detailpanel eingeblendet, das nach der Auswahl eines Termins dessen vollständige Informationen ohne Wechsel der Ansicht zugänglich macht. Im dargestellten Beispiel ist der Termin "Abnahme Tiefbau B36" ausgewählt; das Detailpanel zeigt zunächst eine farbige Kategorienkennzeichnung (Zwischentermin) sowie den Titel und die zugehörige verlinkte Auftragsnummer ("Auftrag \#198241 -- Brückensanierung Pfeiler 2--4"). Es folgen die Eckdaten des Termins (Datum, Uhrzeit mit Dauer in Stunden und Minuten, Ort, beteiligtes Team mit Avataren) sowie ein Bemerkungsfeld mit kontextbezogenen Hinweisen ("Statiker (Hr. Richter) kommt um 11:15. Vermessungsteam steht ab 12:00 bereit.") und eine Liste verknüpfter Buchungen. Die im Detailpanel verlinkten Buchungen führen unmittelbar in die zugehörige Buchungsdetailansicht.
...
- *Anwesenheitszeiten*: Tabellarische Auflistung der Anwesenheitseinträge je Mitarbeiter, gruppierbar nach Tag, Woche oder Monat. Die Erfassung erfolgt -- abgesehen von administrativen Korrekturen -- automatisch durch den nächtlichen Import der Stempelkartendaten.
...
- *Globale Suche*: Eine über die Topbar erreichbare Volltextsuche, die auf sämtliche Hauptentitäten der Anwendung (Aufträge, Projekte, Mitarbeiter, Geräte, Lager, Buchungen) zugreift und die Ergebnisse gruppiert nach Entitätstyp darstellt.
...
Eine offen gehaltene Designentscheidung betrifft die Behandlung administrativer Funktionen wie Datenimport, -export und Berichtserstellung. Die Mockups zeigen diese Funktionen aus Gründen der Übersichtlichkeit als jeweils sichtbare Schaltflächen; im produktiven Einsatz werden sie über die Berechtigungsklasse Rolle ausgeblendet, sobald der angemeldete Mitarbeiter nicht über die entsprechenden Rechte verfügt. Eine alternative Lösung -- die Auslagerung sämtlicher administrativer Funktionen in einen eigenen Administrationsbereich -- wurde verworfen, da hierdurch häufige Wechsel zwischen Bereichen erforderlich würden und die Anwendung als Ganzes weniger zusammenhängend wirken würde.
