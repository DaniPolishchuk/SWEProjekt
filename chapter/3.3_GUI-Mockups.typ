...
Für die Kennzeichnung von Zuständen wurde eine durchgängige Statusfarbpalette eingeführt. Die folgende Tabelle fasst die in allen Ansichten konsistent verwendeten Statuswerte mit ihrer jeweiligen Farbe und ihrem Anwendungsbereich zusammen:

#figure(
  table(
    columns: 3,
    align: (left, left, left),
    table.header([*Farbe*], [*Statuswerte*], [*Anwendungsbereich*]),
    [Blau], [In Arbeit, In Bearbeitung], [Aufträge im aktiven Bearbeitungsstand],
    [Grün], [Verfügbar, Abgeschlossen, Planung], [Positive Zustände, abgeschlossene oder geplante Vorgänge],
    [Gelb / Bernstein], [Pausiert, In Wartung, Teilweise gebucht], [Übergangs- und Wartezustände],
    [Rot], [Defekt, Verzug], [Kritische oder blockierende Zustände],
    [Grau], [Storniert], [Neutrale, deaktivierte oder verworfene Zustände],
  ),
  caption: [Konsistente Statusfarbpalette über alle Ansichten]
) <statusfarben>
...
Die in diesem Kapitel gezeigten Mockups bilden überwiegend die Sicht eines Bau-/Projektleiters mit ergänzenden Verwaltungsrechten ab, da diese kombinierte Rollensicht den breitesten Funktionsumfang im Arbeitsalltag aufweist und sich somit am besten zur Demonstration der vollständigen Anwendung eignet. Funktionen, die laut Berechtigungstabelle ausschließlich dem Verwaltungsmitarbeiter (etwa die Mitarbeiter- und Gruppenverwaltung, das Anlegen neuer Geräte) oder dem Administrator (etwa der Import und Export von Daten oder die Backup-Erstellung) vorbehalten sind, erscheinen in der dargestellten Sicht zwar visuell, sind im produktiven Einsatz jedoch nur für die jeweils berechtigte Rolle tatsächlich sichtbar. In den Sichten anderer Rollen wären diese Schaltflächen entweder ausgeblendet oder deaktiviert. Die Mockups zeigen aus Übersichtsgründen die maximale Funktionsausprägung; die rollenspezifische Reduktion erfolgt zur Laufzeit über die im Klassendiagramm modellierte Klasse `Rolle`.
...
Im darunterliegenden Karteireiterbereich ("Übersicht", "Buchungen", "Wartung", "Ausrüstung", "Standort \& Lager", "Historie") können sämtliche weiterführenden Informationen zum Gerät erschlossen werden. Im standardmäßig geöffneten Reiter "Übersicht" sind die Stammdaten des Exemplars mit interner Nummer und Seriennummer aufgelistet, ergänzt um den Bereich "Zubehör (Baugruppe-Muster)", in dem das dem Gerät zugeordnete Zubehör (Tieflöffel, Hydraulikhammer und ähnliche Anbauteile) verwaltet wird. Diese Zubehörverwaltung realisiert das im Lastenheft (LF 50) geforderte Konzept zur Verwaltung kompatibler Anbaugeräte. Der zugehörige Lager- und Standortbereich ist nicht Teil der Übersicht, sondern wird im eigenen Karteireiter "Standort \& Lager" mit kartografischer Darstellung und kompakter Lageradresse aufgeführt; die Trennung in einen eigenen Reiter trägt der Tatsache Rechnung, dass die räumliche Information im Arbeitsalltag häufig für sich stehen muss (etwa beim Disponieren von Geräten zwischen Baustellen).
