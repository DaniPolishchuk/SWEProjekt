...
Die Mockups verfolgen drei Ziele: hohe Informationsdichte ohne visuelle Überfrachtung, konsistente Gestaltung über alle Ansichten und eine Reduktion auf die tatsächlich benötigten Funktionen. Dieses Spannungsverhältnis wurde je nach Ansicht unterschiedlich aufgelöst -- das Dashboard verdichtet maximal, die Auftragsdetailansicht entzerrt dagegen bewusst.

Eine zentrale Entscheidung war die durchgängige Verwendung kartenbasierter Layouts statt klassischer Tabellen: Karten erlauben eine reichhaltigere Darstellung pro Eintrag (Vorschaubilder, Avatarstapel, Statusinformationen) und sind besser für eine spätere Tablet-Adaption geeignet. Wo Tabellen bei großen Datenmengen effizienter scanbar sind (laufende Aufträge im Dashboard), wurden sie beibehalten. Administrative Funktionen (Import, Export, Berichte) erscheinen in den Mockups aus Übersichtsgründen sichtbar, werden im Betrieb jedoch über die Klasse `Rolle` rollenabhängig ausgeblendet; eine Auslagerung in einen separaten Administrationsbereich wurde verworfen, da sie häufige Bereichswechsel erzwungen hätte. Insgesamt lassen sich sämtliche funktionalen Anforderungen des Lastenhefts durch eine schlanke Oberfläche mit wiederverwendeten Bausteinen (Seitenleiste, Topbar, Statuspillen, Karten) abbilden, was die im Lastenheft geforderte sehr gute Benutzbarkeit erfüllt.
