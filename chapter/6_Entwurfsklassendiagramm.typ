...
Das Entwurfsklassendiagramm ist gemäß Aufgabenstellung 3.3 zu erstellen und stellt gemeinsam mit dem in Kapitel 8 dargestellten Kommunikationsschema die verbindliche Entwurfsgrundlage für die spätere Implementierung dar. Die dortige Beschränkung auf eine einzelne, repräsentative Maske wird an dieser Stelle ergänzt: Während Kapitel 8 die konkrete Ausgestaltung des Model-View-Controller-Musters für die Auftragsübersicht in feinster Detailtiefe zeigt, konzentriert sich das vorliegende Kapitel auf die Gesamtarchitektur des fachlichen Kerns und der Persistenzschicht der Anwendung. Die view- und ereignisspezifischen Klassen aus Kapitel 8 werden hier bewusst nicht wiederholt, um doppelte Modellierungstiefe und die daraus resultierende Unübersichtlichkeit zu vermeiden. Anstelle dessen wird das GUI-Paket im Entwurfsklassendiagramm auf zwei Ankerklassen (`GUIController` und die Schnittstelle `IUpdateEventListener`) reduziert, um den strukturellen Bezug zum Beobachter-Muster und zur zentralen Datenbasis erkennbar zu halten. Konsistent zum Kommunikationsschema aus Kapitel 8 sind die tatsächlichen Empfänger von Datenaktualisierungen View-Klassen wie das `AufträgeÜbersichtPanel`; die Schnittstelle `IUpdateEventListener` wird von diesen View-Klassen implementiert, während der `GUIController` als Bindeglied zwischen View und Model die Beobachter beim `Datenbasis`-Singleton registriert und wieder abmeldet. Ein direkter Beobachter-Anschluss des `GUIController` selbst wird nicht modelliert; das entspricht der in Kapitel 8 gewählten Architektur, in der Datenänderungen aus dem Model unmittelbar an die betroffenen Sichten propagiert werden.
...
- *STATUS-SETZEN* wird durch Setter-Operationen auf den betroffenen Klassen (`Arbeitsauftrag.setStatus(s: AuftragStatus)`, `Buchung.setStatus(s: BuchungStatus)`, `Gerät.setStatus(s: GerätStatus)`) umgesetzt.
- *AUFTRAG-ARCHIVIEREN* entspricht der zusammenfassenden Operation `Arbeitsauftrag.archivieren()`, die vor der eigentlichen Statusänderung die aktiven Buchungen storniert.
...
Gleichzeitig wurde die Modellierungstiefe an mehreren Stellen bewusst begrenzt: Im Datentypen-Paket wurden keine Assoziationslinien eingezeichnet, im GUI-Paket erscheinen lediglich zwei Ankerklassen mit einem Verweis auf das ausführliche Kommunikationsschema in Kapitel 8, und der Beobachter-Mechanismus wird auf die Kernklassen `Datenbasis`, `GUIController` und `IUpdateEventListener` reduziert. Diese bewusste Reduktion folgt der Erkenntnis, dass eine vollständige Modellierung sämtlicher Detailbeziehungen dem Übersichtscharakter des Diagramms zuwidergelaufen wäre. Der aufmerksame Leser findet in Kapitel 8 sowie in den Sequenz- und Aktivitätsdiagrammen die feinere Ausgestaltung, während das vorliegende Diagramm den strukturellen Gesamtblick liefert.
